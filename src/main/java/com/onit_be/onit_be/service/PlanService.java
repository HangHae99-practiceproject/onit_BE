...
public class PlanService {
    ...
    public void createPlan(PlanReqDto planRequestDto, User user) {
        ...
        for(Plan plan : planList){
            //2.이중약속에 대한 처리 .
            if(plan.getPlanDate().compareTo(planRequestDto.getPlanDate()) == 0){
                throw new IllegalArgumentException("해당하는 날짜와 시간에 이미 등록된 일정이 존재합니다!");
            }
            ...
            //4.만약 어떤 일정에 참여한 사람이 회원가입후 일정을 만들려고 했을 떄는 어떻게 예외 처리를 할 수있을지 ? 생각.
        }
